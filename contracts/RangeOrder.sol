...
contract RangeOrder is IERC721Receiver {
    ...
    function setRangeOrder(
        RangeOrderParams calldata _params
    ) external {
        (, int24 tick, , , , , ) = IUniswapV3Pool(_params.pool).slot0();
        int24 tickSpacing = IUniswapV3Pool(_params.pool).tickSpacing();
        require(_params.tickThreshold % tickSpacing == 0, "threshold must be initializable tick");
        int24 lowerTick =
            _params.zeroForOne ? _params.tickThreshold - tickSpacing : _params.tickThreshold;
        int24 upperTick =
            _params.zeroForOne ? _params.tickThreshold : _params.tickThreshold + tickSpacing;
        ...
        (uint256 tokenId,,,) = positions.mint(mintParams);
        positions.approve(address(eject), tokenId);
        eject.schedule(
            IEjectLP.OrderParams({
                tokenId: tokenId,
